...
const [
  browser = "chromium",
  port = "8076",
  output = "output/playwright/m2a-live-weapon-belt",
...
async function waitUntilReady() {
  ...
  throw new Error(
    `Web server did not become ready: ${lastError?.message || "unknown error"}`,
  );
}
...
try {
  ...
  const test = spawn(
    ...
    [
      resolve(
        repository,
        "tests/browser/run_m2a_live_weapon_belt_regression.mjs",
      ),
      browser,
      target,
      output,
