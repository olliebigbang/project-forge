...
function responseFor(requestId, spec) {
  return {
    ...
    weapon_spec: spec,
    interpretation_summary: "Validated semantic fixture; numeric role derivation remains local.",
    confidence: 0.95,
    corrections: [],
    fallback_reason: "",
...
function summarizeScenario(roleCase, scenario, before, opened, current, elapsedToCommitMs, elapsedToSettleMs) {
...
function assertScenario(roleCase, role, spec, evidence) {
  ...
  if (scenario === "group") {
    if (["short_melee", "standard_melee", "long_melee", "straight_ranged"].includes(role.role_id)) assert(targetsHit <= 1, `${roleCase.id}: single-body role hit ${targetsHit} grouped targets`);
    if (["thrown_blast", "direct_blast"].includes(role.role_id)) assert(targetsHit >= 2, `${roleCase.id}: blast did not produce group value`);
    if (role.role_id === "piercing") assert(targetsHit >= 2 && targetsHit <= spec.pierce_count, `${roleCase.id}: piercing group hits ${targetsHit} violate bounded penetration`);
...
try {
  ...
  for (const roleCase of browserCases.filter((entry) => !roleFilter || entry.role_id === roleFilter || entry.id === roleFilter)) {
    ...
    if (roleCase.role_id === "straight_ranged") {
      ...
      await tap(page, (await controls(page)).pattern_buttons[1], `${roleCase.id} PROJECTILE role selector`);
    ...
    for (const scenario of matrix.scenario_order) {
      ...
      const opened = await waitForState(page, "attack_open", attackBefore, `${roleCase.id}/${scenario} commit`);
      const elapsedToCommitMs = Date.now() - started;
      ...
      const evidence = summarizeScenario(roleCase, scenario, isolated, opened, settled, elapsedToCommitMs, elapsedToSettleMs);
