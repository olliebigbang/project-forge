...
func _test_assist_and_horizontal_lock() -> void:
	...
	enemy.global_position = arena_center - Vector2(140.0, 0.0)
	await physics_frame
	_check(_spike.player.assist_target() == null, "assist rejects targets behind facing")
	enemy.global_position = arena_center + Vector2(220.0, 0.0)
	await physics_frame
	_check(_spike.player.assist_target() == enemy, "assist selects a valid forward target")
